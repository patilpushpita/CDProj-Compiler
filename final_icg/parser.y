...
%token RETURN VOID INT IF FOR ELSE BREAK CONTINUE
...
type_specifier
	: VOID {setType();return_num=1;}
	| CHAR {setType();return_num=2;}
	| INT {setType();return_num=0;}
	| FLOAT {setType();return_num=3;}
	| DOUBLE {setType();return_num=4;}
	;

declarator
	: IDENTIFIER {set_f_name(); STMT_DECLARE_FUNC(return_num); set_func_label();}
...
parameter_declaration
	: declaration_specifiers declarator_para
	| declaration_specifiers
	;

declarator_para
	: IDENTIFIER {STMT_DECLARE_ID(); set_data_type();f_arg++;}
	;
...
mainfunction
	...
	| VOID MAIN {set_f_name(); STMT_DECLARE_FUNC(0); set_mainflag();set_func_label();} '(' ')' '{' statementlist '}' {return_flag = 0;} {set_valid(0);}
	; 
*/
...
functioncallstatement
    ...
    | IDENTIFIER {check_func(); goto_func_label(); /*add_func_label();*/} '(' IDENTIFIER ')' ';' {check_f_arg(1);}
    | IDENTIFIER {check_func(); goto_func_label(); /*add_func_label();*/} '(' IDENTIFIER ',' IDENTIFIER ')' ';' {check_f_arg(2);}
    ;
